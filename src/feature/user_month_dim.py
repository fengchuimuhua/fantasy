...
def gen_fea(user_fn, click_fn, order_fn, loan_fn, fea_fn):    
	...
	user_date_loan_df['um_delta_month'] = user_date_loan_df.month - user_date_loan_df.loan_month
	user_date_loan_df['um_left_plannum'] = user_date_loan_df.apply( lambda x : -1 if x['delta_month'] <= 0 else x['plannum'] - x['delta_month'] , axis = 1)
	user_date_loan_df['um_left_balance'] =  user_date_loan_df.apply(lambda x : 0 if x.left_plannum < 0 else x.real_loan_amount / x.plannum * x.left_plannum , axis = 1)
	user_date_loan_df['um_month_need_pay'] = user_date_loan_df.apply(lambda x : 0 if x.left_plannum < 0 else x.real_loan_amount , axis=1)
	...
	user_month_tuned_real_limit_df = agg.groupby(['uid', 'loan_month_x']).agg({"real_loan_amount_y":np.max }).reset_index().sort_values(['uid','loan_month_x']).rename(columns = {"loan_month_x":"month" , "real_loan_amount_y":'um_tuned_limit'})
    ...
    #step 10. output
	user_date_df[['uid','date','um_left_plannum','um_left_balance','um_month_need_pay','um_tuned_limit']].to_csv(fea_fn , index=False)
def merge(a , b):
